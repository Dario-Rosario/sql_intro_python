...
def fill_db():
    ...
    conn = sqlite3.connect('secundaria.db')
    c = conn.cursor()
    group = [('Dario', 18, 1, 'Martinez'),
             ('Marcelo', 18, 1, 'Martinez'),
            ('Carlos', 18, 2, 'Ramos'),
            ('Florencia', 19, 2, 'Ramos'),
            ('Marcela', 19, 2, 'Ramos'),
             ]
    c.executemany("""
...
def search_by_grade(grade):
    ...
    conn = sqlite3.connect('secundaria.db')
    c = conn.cursor()
    c.execute('SELECT id, name, age FROM secundaria WHERE grade = 1')
    data = c.fetchall()
    print(data)
...
def insert(grade):
    ...
    conn = sqlite3.connect('secundaria.db')
    c = conn.cursor()
    c.execute("""INSERT INTO secundaria(name, age, grade, tutor)
                VALUES ('Franco', 21, 1, 'Pedro');""")
...
def modify(id, name):
    ...
    conn = sqlite3.connect('secundaria.db')
    c = conn.cursor()
    rowcount = c.execute("UPDATE secundaria SET name = 'Inove' WHERE id = 2")
